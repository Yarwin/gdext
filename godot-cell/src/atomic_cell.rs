...
use crate::{
    atomic_borrow_state::AtomicBorrowState,
    atomic_guards::{InaccessibleGuard, MutGuard, RefGuard},
};
...
pub struct AtomicGdCell<T>(Pin<Box<GdCellInner<T>>>);

impl<T> AtomicGdCell<T> {
...
pub(crate) struct GdCellInner<T> {
    ...
    pub(crate) borrow_state: AtomicBorrowState,
    /// The mutable state of this cell.
    pub(crate) state: UnsafeCell<AtomicCellState<T>>,
...
impl<T> GdCellInner<T> {
    ...
    pub fn new(value: T) -> Pin<Box<Self>> {
        ...
        // SAFETY: See above.
        unsafe {
            state_ptr.write(UnsafeCell::new(AtomicCellState::new(value_ref)));
        }

        let borrow_state = unsafe { &raw mut (*uninitialized_cell_ptr).borrow_state };
        unsafe {
            borrow_state.write(AtomicBorrowState::new());
        }
    ...
    pub fn borrow(self: Pin<&Self>) -> Result<RefGuard<'_, T>, Box<dyn Error>> {
        self.borrow_state.increment_shared()?;
        // SAFETY: SAFETY: `increment_shared` succeeded, therefore there cannot currently be any accessible mutable
        // references.
    ...
    pub fn borrow_mut(self: Pin<&Self>) -> Result<MutGuard<'_, T>, Box<dyn Error>> {
        self.borrow_state.increment_mut()?;
    ...
    unsafe fn cell_state(&self) -> &AtomicCellState<T> {
    ...
    unsafe fn cell_state_mut(&self) -> &mut AtomicCellState<T> {
        // SAFETY: the underlying `CellState` will not be deallocated as long as Cell itself is alive.
    ...
    pub fn is_currently_bound(self: Pin<&Self>) -> bool {
        self.borrow_state.is_currently_bound()
    }
}

/// Mutable state of the `GdCell`.
#[derive(Debug)]
pub(crate) struct AtomicCellState<T> {
...
impl<T> AtomicGdCell<T> {
    pub fn is_poisoned(&self) -> bool {
        self.0.borrow_state.is_poisoned()
    }
}
...
mod test {
    ...
    fn prevent_mut_mut() {
        ...
        // Cell becomes poisoned and disallows further use.
    ...
    fn prevent_shared_mut() {
        ...
        // Cell becomes poisoned and disallows any further use.
        std::mem::forget(guard1);
    }
    ...
    fn different_inaccessible() {
        ...
        let cell1 = AtomicGdCell::new(VAL1);
        let cell2 = AtomicGdCell::new(VAL2);

...
use crate::atomic_borrow_state::AtomicBorrowState;
use crate::atomic_cell::AtomicCellState;
...
pub struct RefGuard<'a, T> {
    borrow_state: &'a AtomicBorrowState,

    /// The current state of borrows to the borrowed value.
    state: &'a UnsafeCell<AtomicCellState<T>>,
...
impl<T> Drop for RefGuard<'_, T> {
    fn drop(&mut self) {
        ...
        self.borrow_state.decrement_shared().unwrap();
    }
}
...
impl<T> Drop for MutGuard<'_, T> {
    fn drop(&mut self) {
        // The state will become poisoned on failure in `decrement_mut` – simply do nothing in such a case.
        let _ = self.borrow_state.decrement_mut();
    }
}
...
impl<'a, T> InaccessibleGuard<'a, T> {
        ...
        'a: 'b,
    {
        ...
        let ref_count = borrow_state.set_inaccessible()?;
    ...
    fn perform_drop(
        ...
        borrow_state.unset_inaccessible(ref_count).unwrap();
    ...
    pub fn can_drop(&self) -> bool {
        ...
        self.borrow_state.may_unset_inaccessible() || state.stack_depth == self.stack_depth
    }
...
impl<T> Drop for InaccessibleGuard<'_, T> {
    fn drop(&mut self) {
        // Default behavior of drop-logic simply panics and poisons the cell on failure.
        Self::perform_drop(
            self.state,
            self.borrow_state,
            self.ref_count,
